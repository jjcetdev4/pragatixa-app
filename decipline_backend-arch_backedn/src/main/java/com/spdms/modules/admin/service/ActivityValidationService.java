...
public class ActivityValidationService {
    ...
    public String matchXpCategory(String xpCategory) {
        if (xpCategory == null || xpCategory.trim().isEmpty()) return xpCategory;
        List<String> allowedCategories = List.of(
                "Academic", "Skill", "Communication", "Leadership", "Discipline",
                "Placement", "Innovation", "Community", "Sports", "Cultural");
    ...
    public ResponseEntity<ApiResponse<String>> validateXpConfiguration(boolean awardEnabled, boolean penaltyEnabled, Integer awardXp, Integer penaltyXp) {
        if (!awardEnabled && !penaltyEnabled) {
            return ResponseEntity.badRequest().body(ApiResponse.<String>error("At least one XP Configuration (Award or Penalty) must be enabled"));
        }
        if (awardEnabled) {
            if (awardXp == null || awardXp < 0 || (!penaltyEnabled && awardXp == 0)) {
                return ResponseEntity.badRequest().body(ApiResponse.<String>error("Award XP value must be greater than zero when enabled"));
            }
        }
        if (penaltyEnabled) {
            if (penaltyXp == null || penaltyXp <= 0) {
                return ResponseEntity.badRequest().body(ApiResponse.<String>error("Penalty XP value must be greater than zero when enabled"));
            }
        }
        return null; // OK
    }
        ...
        if (awardDays != null && !awardDays.isEmpty()) {
            ...
            for (String day : awardDays) {
                if (validDays.stream().noneMatch(d -> d.equalsIgnoreCase(day.trim()))) {
                    return ResponseEntity.badRequest().body(ApiResponse.<String>error("Invalid Award Day: " + day + ". Must be a valid day of the week."));
                }
            }
        }
        return null; // OK
    ...
    public ResponseEntity<ApiResponse<String>> validateCap(String matchedFrequency, Integer cap) {
        if (!matchedFrequency.equalsIgnoreCase("One Time") && !matchedFrequency.equalsIgnoreCase("Manual")) {
            if (cap == null || cap < 1) {
                return ResponseEntity.badRequest().body(ApiResponse.<String>error("Cap must be at least 1 for recurring frequencies"));
            }
        }
        return null;
    }
