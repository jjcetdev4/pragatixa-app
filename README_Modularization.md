...
The current SPDMS backend uses a flat, functional package structure under `com.spdms` (e.g., all controllers in one package, all entities in another). While functional, this monolithic structure makes the codebase harder to maintain, scale, and understand as the domain grows.
