...
import jjcet.PragatiX.modules.analytics.repository.AttendanceAnalyticsRepository;
...
@Service("baseAttendanceAnalyticsService")
public class AttendanceAnalyticsService {
    ...
    public AnalyticsOverviewDTO getOverview(String yearNo, Long departmentId, Integer stage, Long sectionId,
        ...
        return analyticsRepository.getOverview(effectiveYearNo, departmentId, stage, sectionId, startDate, endDate,
                period);
    }
    ...
    public List<AttendanceTrendDTO> getTrend(String yearNo, Long departmentId, Integer stage, Long sectionId,
            LocalDate startDate, LocalDate endDate, Integer period) {
        String effectiveYearNo = determineYearFilter(yearNo);
        return analyticsRepository.getTrend(effectiveYearNo, departmentId, stage, sectionId, startDate, endDate,
                period);
    }
    ...
    public AttendanceDistributionDTO getDistribution(String yearNo, Long departmentId, Integer stage, Long sectionId,
            LocalDate startDate, LocalDate endDate, Integer period) {
        String effectiveYearNo = determineYearFilter(yearNo);
        return analyticsRepository.getDistribution(effectiveYearNo, departmentId, stage, sectionId, startDate, endDate,
                period);
    }
    ...
    public List<LowAttendanceStudentDTO> getLowAttendanceStudents(String yearNo, Long departmentId, Integer stage,
        ...
        String effectiveYearNo = determineYearFilter(yearNo);
        return analyticsRepository.getLowAttendanceStudents(effectiveYearNo, departmentId, stage, sectionId, startDate,
                endDate, period, threshold);
    }

    public List<GroupedAttendanceDTO> getSectionWiseAttendance(String yearNo, Long departmentId, Integer stage,
            LocalDate startDate, LocalDate endDate, Integer period) {
        String effectiveYearNo = determineYearFilter(yearNo);
        return analyticsRepository.getSectionWiseAttendance(effectiveYearNo, departmentId, stage, startDate, endDate,
                period);
    }
    ...
    public List<AttendanceSummaryRowDTO> getSummaryTable(String yearNo, Long departmentId, Integer stage,
        ...
        String effectiveYearNo = determineYearFilter(yearNo);
        return analyticsRepository.getSummaryTable(effectiveYearNo, departmentId, stage, sectionId, startDate, endDate,
                period);
    }

    public byte[] exportAttendanceReport(String yearNo, Long departmentId, Integer stage, Long sectionId,
            LocalDate startDate, LocalDate endDate, Integer period) {
        String effectiveYearNo = determineYearFilter(yearNo);
        List<AttendanceExportDTO> rawData = analyticsRepository.getExportData(effectiveYearNo, departmentId, stage,
                sectionId, startDate, endDate, period);
        ...
        try (org.apache.poi.xssf.usermodel.XSSFWorkbook workbook = new org.apache.poi.xssf.usermodel.XSSFWorkbook()) {
            ...
            headerStyle
                    .setFillForegroundColor(org.apache.poi.ss.usermodel.IndexedColors.LIGHT_CORNFLOWER_BLUE.getIndex());
            ...
            for (AttendanceExportDTO row : rawData) {
                ...
                String sec = row.getSectionName() != null && !row.getSectionName().isEmpty() ? row.getSectionName()
                        : "None";
            ...
            int rowIdx = 0;
            String[] headers = { "S.No", "Register Number", "Student Name", "Department", "Section", "Date", "P1", "P2",
                    "P3", "P4", "P5", "P6", "P7", "P8", "Present Count", "Absent Count", "Attendance %" };

            for (java.util.Map.Entry<String, java.util.Map<String, java.util.Map<String, StudentRow>>> deptEntry : deptSecStudentMap
                    .entrySet()) {
                ...
                sheet.addMergedRegion(
                        new org.apache.poi.ss.util.CellRangeAddress(rowIdx - 1, rowIdx - 1, 0, headers.length - 1));
                ...
                for (java.util.Map.Entry<String, java.util.Map<String, StudentRow>> secEntry : deptEntry.getValue()
                        .entrySet()) {
                    ...
                    if (!"None".equals(sec)) {
                        ...
                        sheet.addMergedRegion(new org.apache.poi.ss.util.CellRangeAddress(rowIdx - 1, rowIdx - 1, 0,
                                headers.length - 1));
                    }
                    ...
                    for (StudentRow s : secEntry.getValue().values()) {
                        ...
                        if (pct >= 95)
                            pctCell.setCellStyle(pctGreenStyle);
                        else if (pct >= 75)
                            pctCell.setCellStyle(pctOrangeStyle);
                        else
                            pctCell.setCellStyle(pctRedStyle);
                    }
                }
