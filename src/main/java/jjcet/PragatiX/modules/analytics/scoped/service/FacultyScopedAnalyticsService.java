...
@Service
public class FacultyScopedAnalyticsService {
    ...
    public FacultyScopedAnalyticsService(AnalyticsService analyticsService, AuthUtils authUtils,
    ...
    public AllInOneDashboardDto getScopedDashboard(String yearNo, Integer stage, LocalDate startDate, LocalDate endDate) {
        ...
        Long sectionId = currentUser.getSection() != null ? currentUser.getSection().getId() : null;

        if (departmentId == null || sectionId == null) {
            Faculty faculty = facultyRepository.findByUserUsername(currentUser.getUsername()).orElse(null);
            if (faculty != null) {
                if (departmentId == null && faculty.getDepartment() != null) {
                    departmentId = faculty.getDepartment().getId();
                }
                if (sectionId == null && faculty.getSection() != null) {
                    sectionId = faculty.getSection().getId();
                }
            }
        }
        ...
        List<AttendanceCalendarDto> attendanceCalendar = analyticsService.getAttendanceCalendar(yearNo, departmentId, stage, sectionId, startDate, endDate, null);
        ...
        dto.setAttendanceSummary(attendanceAnalyticsService.getAttendanceSummaryByDate(yearNo, endDate != null ? endDate : LocalDate.now(), departmentId, stage, sectionId, null));
