...
import com.spdms.modules.student.service.StudentMapper;
...
public class LeaderboardService {
    ...
    public LeaderboardService(StudentRepository studentRepository, UserRepository userRepository, YearRepository yearRepository, DepartmentRepository departmentRepository, SectionRepository sectionRepository, StudentMapper studentMapper) {
    ...
    public ApiResponse<List<StudentResponse>> getLeaderboard(Long yearId, Long departmentId, Long sectionId) {
        ...
        boolean isCc = currentUser != null && currentUser.getSubRoles().stream()
                .anyMatch(sr -> sr.getName().trim().equalsIgnoreCase("CC") || sr.getName().trim().equalsIgnoreCase("CLASS_COORDINATOR"));
        ...
        if (finalDeptId != null) {
            students = students.stream().filter(s -> s.getDepartment() != null && s.getDepartment().getId().equals(finalDeptId)).collect(Collectors.toList());
        }
        
        final Long finalYearId = targetYearId;
        if (finalYearId != null) {
            students = students.stream().filter(s -> s.getYearRef() != null && s.getYearRef().getId().equals(finalYearId)).collect(Collectors.toList());
        }
        
        final Long finalSectionId = targetSectionId;
        if (finalSectionId != null) {
            students = students.stream().filter(s -> s.getSection() != null && s.getSection().getId().equals(finalSectionId)).collect(Collectors.toList());
        }
    ...
    private Long resolveYearId(String yearStr) {
        ...
        if (yearNo != null) {
            ...
            if (year != null) return year.getId();
        }
        return null;
    }
    ...
    public ApiResponse<FilterOptionsDto> getFilters(Long yearId, Long departmentId) {
        ...
        if (isTeacher && !isAdmin && isCc) {
            ...
            if (currentUser.getYear() != null) {
                ...
                if (yId != null) yearFilters.add(new FilterOptionsDto.FilterItem(yId.toString(), currentUser.getYear()));
            }
            if (currentUser.getDepartment() != null) {
                deptFilters.add(new FilterOptionsDto.FilterItem(currentUser.getDepartment().getId().toString(), currentUser.getDepartment().getName()));
            }
            if (currentUser.getSection() != null) {
                sectionFilters.add(new FilterOptionsDto.FilterItem(currentUser.getSection().getId().toString(), currentUser.getSection().getSectionName()));
            }
        ...
        if (departmentId != null) {
            ...
            secs.forEach(s -> sectionFilters.add(new FilterOptionsDto.FilterItem(s.getId().toString(), s.getSectionName())));
        }
