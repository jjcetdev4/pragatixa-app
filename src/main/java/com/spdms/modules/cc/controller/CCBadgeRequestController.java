...
public class CCBadgeRequestController {
    ...
    public ResponseEntity<ApiResponse<List<BadgeRequestDto>>> getCCRequests() {
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to fetch CC badge requests", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<BadgeRequestDto>> approveRequest(@PathVariable Long id) {
        ...
        // For simplicity, we assume the frontend only shows their students, but an extra check in the service would be ideal.
        try {
